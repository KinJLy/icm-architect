...
- `chart-of-accounts.md` — the category list transactions get coded to. Read by `02_Reconcile` and `03_Ledger`.
- `accounts.md` — the registry of bank accounts (number, status, ledger file). Read by every stage instead of hardcoding account/ledger names. **In progress** — 2 of 4 accounts fully registered, see file for what's outstanding.
